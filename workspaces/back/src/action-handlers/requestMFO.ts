...
export async function requestMFO (message: TelegramMessage, bot: any) {

    await DB.mongo.collection('users')
        .updateOne(
            {tg_id: message.from.id},
            {$set: {
                    'shop.mfo': message.text,
                    action: {
                        type: Actions.REQUEST_OKED,
                    }
                }}
        );
    ...
    bot.sendMessage(message.from.id, 'Введите <b>ОКЭД</b>', Keyboard.empty())
}
